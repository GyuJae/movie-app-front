...
class TvsService {
  ...
  public getTvs = async (category: TvCategory): Promise<ITVResult> => {
    const response = await this.makeApiCall<ITVResult>(`tv/${category}?api_key=${this.REACT_APP_MOVIE_API_KEY}`)
    if (!response.results) {
  ...
  public getPageTVs = async ({
    ...
    const response = await this.makeApiCall<ITVResult>(
      `tv/${category}?api_key=${this.REACT_APP_MOVIE_API_KEY}&page=${pageParam}`
    )
    if (!response.results) {
  ...
  public getTV = async (id: string) => {
    const response = await this.makeApiCall<ITVDetail>(`tv/${id}?api_key=${this.REACT_APP_MOVIE_API_KEY}`)
    if (!response.id) {
  ...
  public getRecommendations = async (id: string) => {
    const response = await this.makeApiCall<ITVResult>(
      `tv/${id}/recommendations?api_key=${this.REACT_APP_MOVIE_API_KEY}`
    )
    if (!response.results) {
  ...
  public getCredits = async (id: string) => {
    const response = await this.makeApiCall<ITVCredits>(`tv/${id}/credits?api_key=${this.REACT_APP_MOVIE_API_KEY}`)
    if (!response.id) {
  ...
  public getSimilar = async (id: string) => {
    const response = await this.makeApiCall<ITVResult>(`tv/${id}/similar?api_key=${this.REACT_APP_MOVIE_API_KEY}`)
    if (!response.results) {
      throw new Error('Movies not found')
    }
    return response
  ...
  public getGenres = async () => {
    const response = await this.makeApiCall<ITVGenres>(`genre/movie/list?api_key=${this.REACT_APP_MOVIE_API_KEY}`)
    if (!response.genres) {
  ...
  public getSearch = async ({ query, pageParam = 1 }: { query: string; pageParam?: number }) => {
    const response = await this.makeApiCall<ITVResult>(
      `search/tv?api_key=${this.REACT_APP_MOVIE_API_KEY}&query=${query}&page=${pageParam}`
    )
    if (!response.results) {
      throw new Error('Genres not found')
    }
